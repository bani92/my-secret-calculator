    ...
    <template v-else>
      <section class="panel">
        ...
        <div class="yearly-stat-chart" data-testid="yearly-expense-chart" aria-label="년도별 총 지출">
          <button
            v-for="stat in store.yearlyExpenseStats"
            :key="stat.year"
            type="button"
            class="year-stat-row"
...
const monthlyStats = computed(() => store.getMonthlyExpenseStats(selectedYear.value));
const maxYearTotal = computed(() => Math.max(...store.yearlyExpenseStats.map((stat) => stat.total), 0));
const maxMonthTotal = computed(() => Math.max(...monthlyStats.value.map((stat) => stat.total), 0));
const selectedYearTotal = computed(
  () => store.yearlyExpenseStats.find((stat) => stat.year === selectedYear.value)?.total ?? 0
);
