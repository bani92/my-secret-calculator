...
If you already ran `schema.sql`, run the new `income_records` table definition, the updated `replace_budget_data` function, and its revoke/grant statements in the SQL Editor. Rerun the full schema only after confirming all four budget tables are empty.
